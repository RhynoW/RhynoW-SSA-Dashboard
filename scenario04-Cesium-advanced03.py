#!/usr/bin/env python3
"""
scenario04-Cesium-advanced03.py — 太空態勢儀表板（進階版）+ 台北覆蓋分析 + 時間軸
====================================================================================
Port 5013
...
_SIGMA_R_KM: float    = float(os.getenv("SIGMA_R_KM", "0.1"))   # 位置 1-sigma (km)
_SIGMA_T_KM: float    = float(os.getenv("SIGMA_T_KM", "0.5"))   # 切向 1-sigma
_SAT_RADIUS_KM: float = 0.005  # 衛星等效硬體半徑
...
if REDIS_URL:
    try:
        ...
        _redis_client.ping()
        _USE_REDIS = True
        logger.info("Redis 快取已啟用: %s", REDIS_URL)
...
def _cache_set(key: str, value: dict, ttl: int = STATS_TTL) -> None:
    """寫入 Redis（若啟用）；失敗時靜默降級。"""
    if _USE_REDIS and _redis_client:
        try:
            _redis_client.setex(key, ttl, _json.dumps(value, default=str))
        except Exception as exc:
...
_OVERPASS_CATS_FILE = _SCRIPT_DIR / "overpass_cats.yaml"
...
def _load_overpass_cats() -> dict[str, dict]:
    ...
    if _OVERPASS_CATS_FILE.exists():
        try:
            import yaml
            with _OVERPASS_CATS_FILE.open(encoding="utf-8") as f:
...
_OVERPASS_CATS: dict[str, dict] = _load_overpass_cats()
...
_BASEMAP_CATALOG: dict[str, dict] = {
    ...
    "2k": {
        ...
        "candidates": [_SCRIPT_DIR / "data" / "textures" / "land_shallow_topo_2048.jpg"],
    },
    "topo": {
        "file": "world_topo_bathy_5400.jpg", "label": "地形+海底地形 5.4K",
        "credit": "NASA Visible Earth", "type": "single",
        "candidates": [_SCRIPT_DIR / "data" / "textures" / "world_topo_bathy_5400.jpg"],
    },
    "natural_earth": {
        "file": "", "label": "Natural Earth II",
        "credit": "Natural Earth / CesiumJS (已內建)", "type": "tms",
        "tms_url": "/cesium/Assets/Textures/NaturalEarthII/",
        "candidates": [_SCRIPT_DIR / "data" / "cesium" / "Assets" / "Textures" / "NaturalEarthII" / "tilemapresource.xml"],
    },
}
...
def _compute_pc_chan(
    miss_km: float,
    sigma_r: float = _SIGMA_R_KM,
    sigma_t: float = _SIGMA_T_KM,
    r_sat:   float = _SAT_RADIUS_KM,
) -> float:
...
def _get_globe_texture() -> bytes | None:
    ...
    try:
        ...
        resp = requests.get(_GLOBE_TEXTURE_CDN, timeout=10)
    ...
    logger.error("Globe 貼圖無法取得（本地: %s）", _GLOBE_TEXTURE_LOCAL)
    return None
...
def get_stats() -> dict[str, Any]:
    ...
    _cache_set("stats", _stats_cache, ttl=STATS_TTL)
    return _stats_cache


def get_db_info() -> dict[str, Any]:
    global _db_info_cache, _db_info_loaded_at
    cached = _cache_get("db_info")
    if cached:
        return cached
    ...
    _cache_set("db_info", result, ttl=_DB_INFO_TTL)
    return result
...
def _gmst_iau2006(jd: float, fr: float) -> float:
...
def predict_taipei_passes(
    ...
    if est_mb > _MAX_PASSES_MATRIX_MB:
        logger.warning(
            "過頂預報矩陣估計 %.0f MB > 上限 %d MB，自動縮減衛星數",
            est_mb, _MAX_PASSES_MATRIX_MB,
        )
        safe_n = int(_MAX_PASSES_MATRIX_MB * 1024 ** 2 / (n_steps * 3 * 8))
        # 按比例縮減各類別
...
# ─────────────────────────────────────────────────────────────────────────────
# 過頂預報非同步執行器（防止大量衛星矩陣運算阻塞 Flask main thread）
# ─────────────────────────────────────────────────────────────────────────────
_passes_executor: concurrent.futures.ThreadPoolExecutor = (
    concurrent.futures.ThreadPoolExecutor(
        max_workers=2, thread_name_prefix="passes_worker"
    )
)
_passes_future_cache: dict[str, concurrent.futures.Future] = {}
_passes_result_cache: dict[str, dict] = {}
_passes_lock = threading.Lock()
_MAX_PASSES_MATRIX_MB = 512


def _passes_cache_key(hours: float, step_sec: float, mask_deg: float) -> str:
...
def get_conjunctions(threshold_km: float = _CONJ_THRESHOLD_KM) -> dict[str, Any]:
    ...
    _cache_set(cache_key, _conj_cache, ttl=_CONJ_TTL)
    return _conj_cache
...
#basemap-bar{border-top:1px solid #21262d;background:#0d1117;padding:5px 0 3px;flex-shrink:0}
#basemap-bar .sec{padding:3px 12px 2px;font-size:10px;color:#484f58;text-transform:uppercase;letter-spacing:.8px}
.bm-btn{display:flex;align-items:center;gap:7px;padding:4px 12px;cursor:pointer;
  color:#8b949e;font-size:11px;transition:.12s;user-select:none;border:none;background:transparent;
  width:100%;text-align:left}
.bm-btn:hover{background:#1c2128;color:#c9d1d9}
.bm-btn.active{color:#58a6ff}
.bm-btn .bm-dot{width:8px;height:8px;border-radius:50%;border:1.5px solid #484f58;flex-shrink:0;transition:.12s}
.bm-btn.active .bm-dot{background:#58a6ff;border-color:#58a6ff}
.bm-btn.unavail{opacity:.35;cursor:not-allowed}
...
#logo-overlay{position:absolute;bottom:8px;right:8px;width:120px;height:120px;
...
def create_app() -> Flask:
    ...
    def api_taipei_passes():
        ...
        cache_key = _passes_cache_key(hours, step, mask)
        with _passes_lock:
            if cache_key in _passes_result_cache:
                data = dict(_passes_result_cache[cache_key])
            ...
            if cache_key in _passes_future_cache:
                fut = _passes_future_cache[cache_key]
                if not fut.done():
                    return jsonify({"status": "computing", "retry_after_sec": 5}), 202
                try:
                    result = fut.result()
                    _passes_result_cache[cache_key] = result
                    del _passes_future_cache[cache_key]
                    resp = make_response(_json.dumps(result, ensure_ascii=False).encode("utf-8"))
                    resp.headers["Content-Type"]  = "application/json; charset=utf-8"
                    resp.headers["Cache-Control"] = "public, max-age=300"
                    return resp
                except Exception as exc:
            ...
            fut = _passes_executor.submit(
                predict_taipei_passes, hours, step, mask
            )
            _passes_future_cache[cache_key] = fut
    ...
    @app.get("/api/globe_texture/<name>")
    def api_globe_texture_named(name: str):
        """動態底圖服務：依 name 回傳 _BASEMAP_CATALOG 中對應的圖檔（single 類型）。"""
        ...
        if name not in _texture_file_cache:
            try:
                ...
                logger.info("底圖 '%s' 載入: %d bytes (%s)", name, len(_texture_file_cache[name]), path.name)
            except Exception as exc:
                logger.error("底圖讀取失敗 %s: %s", path, exc)
                return make_response("底圖讀取失敗", 500)
        data = _texture_file_cache[name]
        ct = "image/png" if data[:4] == b"\x89PNG" else "image/jpeg"
        resp = make_response(data)
        resp.headers["Content-Type"]  = ct
        resp.headers["Cache-Control"] = "public, max-age=604800"
    ...
    def api_textures():
        """回傳可用底圖清單，含 type/tms_url/max_zoom 與是否可用。"""
        result = []
    ...
    @app.post("/api/admin/reload_cats")
    def api_reload_cats():
        """POST /api/admin/reload_cats — 熱重載 overpass_cats.yaml，不需重啟服務。"""
        global _OVERPASS_CATS
        _OVERPASS_CATS = _load_overpass_cats()
        return jsonify({"status": "ok", "categories": list(_OVERPASS_CATS.keys())})
